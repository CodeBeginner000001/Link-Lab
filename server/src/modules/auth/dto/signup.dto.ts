...
export class VerifyOtpDto {
  @IsString({ message: 'OTP: OTP must be a string' })
  @IsNotEmpty({ message: 'OTP: OTP is required' })
  @Length(6, 6, { message: 'OTP: OTP must be exactly 6 digits' })
  @Matches(/^\d{6}$/, { message: 'OTP: OTP must contain only numbers' })
  otp!: string;
}

export class LoginDto {
  @IsEmail({}, { message: 'Email: Please provide a valid email address' })
  @IsNotEmpty({ message: 'Email: Email is required' })
  @MaxLength(255)
  email!: string;
