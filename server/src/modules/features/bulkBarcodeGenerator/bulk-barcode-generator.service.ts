...
import {
  ...
  BULK_BARCODE_DOWNLOAD_READ_BATCH_SIZE,
  BULK_BARCODE_INSERT_CONCURRENCY,
  BULK_BARCODE_INSERT_BATCH_SIZE,
...
export class BulkBarcodeGeneratorService {
  constructor(
    ...
    @InjectModel(BulkBarcodeItem.name)
    private readonly bulkBarcodeItemModel: Model<BulkBarcodeItemDocument>,
  ...
  async generate(
    ...
    let fileName = this.buildAutoGeneratedFileName(dto);
    ...
    try {
      ...
      await Promise.all([
        this.bulkBarcodeModel.deleteOne({ _id: bulkBarcode._id }).exec(),
        this.bulkBarcodeItemModel
          .deleteMany({ bulkBarcodeId: bulkBarcode._id, userId })
          .exec(),
      ]);
      throw error;
    }
  ...
  private buildAutoGeneratedFileName(dto: GenerateBulkBarcodeDto) {
    const format = dto.autoFormat ?? BarcodeFormat.CODE128;

    return `auto-generated-${format.toLowerCase()}-barcodes`;
  }
  ...
  private async *getBulkBarcodeItems(bulkBarcode: BulkBarcodeDocument) {
    const cursor = this.bulkBarcodeItemModel
      .find({
        bulkBarcodeId: bulkBarcode._id,
        userId: bulkBarcode.userId,
      })
      .sort({ row: 1 })
      .batchSize(BULK_BARCODE_DOWNLOAD_READ_BATCH_SIZE)
      .lean()
      .cursor();
    ...
    const legacyItems = bulkBarcode.get('items') as
      | Array<
          Pick<
            BulkBarcodeItem,
            | 'row'
  ...
  private async insertBulkBarcodeItemsInBatches(
    items: Array<
      Pick<
        BulkBarcodeItem,
        | 'row'
    ...
    for (
      let index = 0;
      index < items.length;
      index += BULK_BARCODE_INSERT_BATCH_SIZE
    ) {
      batches.push(items.slice(index, index + BULK_BARCODE_INSERT_BATCH_SIZE));
    }

    for (
      let index = 0;
      index < batches.length;
      index += BULK_BARCODE_INSERT_CONCURRENCY
    ) {
      await Promise.all(
        batches
          .slice(index, index + BULK_BARCODE_INSERT_CONCURRENCY)
          .map((batch) =>
            this.bulkBarcodeItemModel.insertMany(batch, { ordered: false }),
          ),
      );
    }
  }
